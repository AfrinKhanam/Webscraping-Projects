...
class ESPostProcessing:
    ...
    def get_document_list(self,document):
        self.first_doc = document['ES_RESULT']['DOCUMENTS'][0]
        self.second_doc = document['ES_RESULT']['DOCUMENTS'][1]
        self.third_doc = document['ES_RESULT']['DOCUMENTS'][2]
        return document['ES_RESULT']['DOCUMENTS']

    # all docs maintitles are same
    def all_maintitles_same(self,document):
        if self.get_document_list(document).count(self.first_doc['stemmed_main_title']) == len(self.get_document_list(document)):
            return True
        return False
    
    #all docs maintitles are different
    def all_maintitles_different(self,document):
        if self.get_document_list(document).count(self.first_doc['stemmed_main_title']) == 1:
            return True
        return False

    #if any 2 docs are same
    def is_last_two_docs_same(self):
        if self.second_doc['stemmed_main_title'] == self.third_doc['stemmed_main_title']:
            return True
        return False

    def create_buckets(self,document):
        bucket1 = []
        bucket2 = []
        bucket3 = []

        if self.all_maintitles_same(document):
            print("All docs main title is same")
            bucket1 = self.get_document_list(document)
            # return 
        elif  self.all_maintitles_different(document):
            print("All docs main title is different")
            bucket1.append(self.first_doc)
            bucket2.append(self.second_doc)
            bucket3.append(self.third_doc)
        else:
            print('first doc --> ',self.second_doc['stemmed_main_title'])
            print('second doc --> ',self.second_doc['stemmed_main_title'])
            print('third doc --> ',self.third_doc['stemmed_main_title'])

            #if last 2 docs are same then bucket1 will have 1st doc and bucket2 will have last 2 docs
            if self.is_last_two_docs_same():
                bucket1.append(self.first_doc)
                bucket2.append(self.second_doc)
                bucket2.append(self.third_doc)
            else:
                #if 1st two docs are same then bucket1 will have 1st two docs and bucket2 will have last doc
                bucket1.append(self.first_doc)
                bucket1.append(self.second_doc)
                bucket2.append(self.third_doc)
        return bucket1,bucket2,bucket3

    def matched_words_list(self,query,string):
        query_list = set(self.create_regex(query))
        string_list = set(self.create_regex(string))
        matched_words = query_list.intersection(string_list)
        return matched_words

    
    def match_query_with_maintitle(self,document):
        matched_words_count = []
        for _,doc in enumerate(self.get_document_list(document)):
            matched_words = self.matched_words_list(query=document['PARSED_QUERY_STRING'], string=doc['stemmed_main_title'])
            doc['matched_maintitle_words'] = list(matched_words)
            doc['matched_maintitle_words_count'] = len(matched_words)
            matched_words_count.append(len(matched_words))
            # print(json.dumps(document,indent=4))
        return matched_words_count
    
    def sort_documents(self,document,matched_words_count):
        zipped=zip(self.get_document_list(document),matched_words_count)
        zipped=list(zipped)
        sorted_docs = sorted(zipped,key=lambda x: x[1], reverse=True)
        return sorted_docs
    
    def main_title_prioritization(self,document):

        #returns the count of matched words with respect to main title
        matched_words_count = self.match_query_with_maintitle(document)
        
        #sort the docs in descending order based on matched words count in each document
        sorted_docs = self.sort_documents(document=document, matched_words_count = matched_words_count)
        # --------------------------------------------------------------------------------------
        #sorted docs assigning to the documents list
        document_list = []
        ...
        #prioritize document based on main title
        bucket1,bucket2,bucket3 = self.create_buckets(document)
        # print("buck1 \n"+json.dumps(bucket1,indent=4))
        # print("buck2 \n"+json.dumps(bucket2,indent=4))
        # print("buck3 \n"+json.dumps(bucket3,indent=4))
        return document,bucket1,bucket2,bucket3

#------------------------------------------------------------------------------------------
    def match_query_with_subtitle(self,document):
        matched_words_count = []
        for _,doc in enumerate(self.get_document_list(document)):
            matched_words = self.matched_words_list(query=document['PARSED_QUERY_STRING'], string=doc['stemmed_title'])
            doc['matched_subtitle_words'] = list(matched_words)
            doc['matched_subtitle_words_count'] = len(matched_words)
            matched_words_count.append(len(matched_words))
            print(json.dumps(document,indent=4))
        return matched_words_count
    
    # def remove_prepositions_from_subtitle(document):



    def subtitle_prioritization(self,bucket1,bucket2,bucket3,document):
        print(f'bucket1------------> \n {json.dumps(bucket1,indent=4)}')
        print(f'bucket2------------> \n {json.dumps(bucket2,indent=4)}')
        print(f'bucket3------------> \n {json.dumps(bucket3,indent=4)}')
        ...
        if len(bucket1) == 3:
            pass
            # print(f'all docs main title same------------> \n {json.dumps(bucket1,indent=4)}')
            # matched_words_count = self.match_query_with_subtitle(bucket1)
            # print(matched_words_count)

        elif bucket1 != [] and (bucket2 != [] and bucket3 != []):
            print(f'all docs main title different------------> \n {json.dumps(bucket1,indent=4)},{json.dumps(bucket2,indent=4)},{json.dumps(bucket3,indent=4)}')
        else:
            print(f'any two docs main title same------------> \n {json.dumps(bucket1,indent=4)},{json.dumps(bucket2,indent=4)}')
            
        # matched_words_count = self.match_query_with_subtitle(document)
        # sorted_docs = self.sort_documents(document,matched_words_count)
        # print(json.dumps(sorted_docs,indent=4))
        # document_list = []
        # for doc in sorted_docs:
        #     document_list.append(doc[0])
        # document['ES_RESULT']['DOCUMENTS'] = document_list
